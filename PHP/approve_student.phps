...
if(!mysqli_connect_errno())
{
	...
	if ($result-> num_rows > 0) { //did it return anything
		while ($row = $result->fetch_assoc()) //keep pulling more
		{
			...
			{
				echo "user: " . $row["id"]. " - First Name: ". $row["FirstName"]. " - Last Name: ". $row["LastName"]. " - Status: ". $row["status"]. "<br>";
			}
		}
	} else{
		echo "0 results";
...
?>
<br>
Please Enter the WNumber above for the user you wish to approve
<br>
<form action="approve_studentRCV.php" method="post">
